...
/**
 * Excel(xls后缀)文件操作工具类，包括读、合并等功能
 *
 * @author LinQiang
 * @date 2016-12-8 17:54:56
 * @version V1.0
 */
public class PoiImport {
    ...
    public static void main(String[] args) {
        PoiImport eu = new PoiImport();

        //从第1行开始读取
        eu.setStartReadPos(0);

        String src_xlspath = "C:\\Users\\Administrator\\Desktop\\智慧宫数据库设计.xls";
        List<Row> rowList;
        try {
    ...
    public PoiImport(String excelPath) {
     ...
     * @return
     */
    public PoiImport RestoreSettings() {
        PoiImport instance = new PoiImport(this.excelPath);
        return instance;
    }
     ...
     * @throws IOException
     */
    public List<Row> readExcel(File file, String path) throws IOException {
        ...
        try {
            ...
            if ("xls".equals(ext)) {	//使用xls方式读取
                ...
                out("您要操作的文件没有扩展名，正在尝试以xls,xlsx方式读取...");
                try {
                    return readExcel_xls(file);
                } catch (IOException e1) {
                    out("尝试以xls方式读取，结果失败！");
    ...
    public List<Row> readExcel_xlsx(File file) throws IOException {
        ...
        XSSFWorkbook wb = null;// 用于Workbook级的操作，创建、删除Excel
        List<Row> rowList = new ArrayList<Row>();
