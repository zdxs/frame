/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.core.dao;

import com.core.bean.CoreModel;
import com.core.common.sql.WherePrams;
import com.core.utils.Pagination;
import java.io.Serializable;
import java.util.List;
import java.util.Map;

/**
 * 基础查询
 *
 * @author xiaosun
 * @param <T>
 * @param <PK>
 */
public interface CoreDao<T extends CoreModel, PK extends Serializable> {

    /**
     * 添加不为空的记录（只将不为空字段入库，效率高）
     *
     * @param CoreModel
     * @return 受改变的记录数
     */
    public int addLocal(T CoreModel);

    /**
     * 记录添加（所有字段入库，效率中）
     *
     * @param CoreModel
     * @return
     */
    public int add(T CoreModel);

    /**
     * 通过主键获取某个记录
     *
     * @param id 主键
     * @return CoreModel
     */
    public T get(PK id);

    /**
     * 通过主键获取某个记录
     *
     * @param id 主键
     * @return CoreModel
     */
    public T getById(PK id);

    /**
     * 通过主键获取某个字段的值
     *
     * @param id
     * @param fileName
     * @return
     */
    public Serializable getField(PK id, String fileName);

    /**
     * 条件获取一条记录
     *
     * @param where
     * @return where
     */
    public T get(WherePrams where);

    /**
     * 条件获取某个记录字段
     *
     * @param where
     * @param fileName
     * @return
     */
    public Serializable getFile(WherePrams where, String fileName);

    /**
     * 条件查询列表
     *
     * @param where 条件表达式
     * @return CoreModel列表
     */
    public List<T> list(WherePrams where);

    /**
     * 根据指定的sql查询
     *
     * @param sql sql语句
     * @return CoreModel列表
     */
    public List<T> listSql(String sql);

    /**
     * 查询某个字段列表
     *
     * @param where 条件表达式
     * @param fileName 要查询的字段
     * @return
     */
    public Serializable[] listFile(WherePrams where, String fileName);

    /**
     * 查询某些字段
     *
     * @param where 条件表达式
     * @param files 要查询的字段集
     * @return 查询的CoreModel字段列表
     */
    public List<Map<String, Serializable>> listFiles(WherePrams where, String[] files);

    /**
     * 更新不为null的CoreModel字段
     *
     * @param CoreModel
     * @return 受影响的行数
     */
    public int updateLocal(T CoreModel);

    /**
     * 更新CoreModel的所有字段
     *
     * @param CoreModel
     * @return 受影响的行数
     */
    public int update(T CoreModel);

    /**
     * 条件更新不为null的字段
     *
     * @param CoreModel
     * @param where 条件表达式
     * @return 受影响的行数
     */
    public int updateLocal(T CoreModel, WherePrams where);

    /**
     * 条件更新所有字段
     *
     * @param CoreModel
     * @param where 条件表达式
     * @return 受影响的行数
     */
    public int update(T CoreModel, WherePrams where);

    /**
     * 删除某个记录
     *
     * @param id 主键
     * @return 受影响的行数
     */
    public int del(PK id);

    /**
     * 条件删除某个记录
     *
     * @param where 条件表达式
     * @return 受影响的行数
     */
    public int del(WherePrams where);

    /**
     * 自定义sql查询
     *
     * @param sql 用于执行查询的Sql
     * @return 结果集合
     */
    public List<Map<String, Object>> listBySql(String sql);

    /**
     * 执行自定义sql
     *
     * @param sql 用于执行的Sql
     * @return 受影响的行数
     */
    public int excuse(String sql);

    /**
     * 获取指定条件的记录数
     *
     * @param where 条件表达式
     * @return 查询到的记录数
     */
    public long count(WherePrams where);

    /**
     * 获取指定条件的记录数没有分页
     *
     * @param where 条件表达式
     * @return 查询到的记录数
     */
    public long countNoPage(WherePrams where);

    /**
     * 获取对应表中的记录数
     *
     * @return 表中的条数
     */
    public long size();

    /**
     * 是否存在字段相同的记录（ID以及不为空的字段除外）
     *
     * @param CoreModel 参照实体
     * @return
     */
    public boolean isExist(T CoreModel);

    /**
     * 是否存在指定条件的记录
     *
     * @param where 条件表达式
     * @return
     */
    public boolean isExist(WherePrams where);

    /**
     * 内查询
     *
     * @param fileName 用于内查询的字段
     * @param values 字段的值
     * @return 查询到的结果集
     */
    public List<T> in(String fileName, Serializable[] values);

    /**
     * 获得下一个序列的值
     *
     * @return
     */
    public long nextId();

    /**
     * 分页模型查询所有分页
     *
     * @param where
     * @param pageIndex
     * @param pageSize
     * @return
     */
    public Pagination<T> queryPaginationList(WherePrams where, int pageIndex, int pageSize);

    /**
     * 从表数据删除，
     *
     * @param field 数据库从表字段
     * @param values 数据库从表字段值
     * @return
     */
    public int deleteTmp(String field, String values);
}
